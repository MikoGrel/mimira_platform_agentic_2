...
export function FileGroup({
  ...
  return (
    <Accordion
      ...
      collapsible
      key={zipName}
      defaultValue={defaultOpen ? zipName : undefined}
    >
      <AccordionItem
        value={zipName}
        className="bg-white rounded-xl px-4 border border-b-1!"
      >
        <AccordionTrigger>
          ...
          <div className="space-y-2">
            {showDownloadAll && files.length > 1 && (
              <Button
                size="sm"
                variant="bordered"
                className="flex items-center gap-1.5 text-xs h-8 ml-auto bg-white"
                onPress={() =>
                  downloadAllFiles(
                    zipName,
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            {" "}
                            {relativeToNow(new Date(file.created_at))}
                          </span>
                        </div>
                        {file.signable && (
